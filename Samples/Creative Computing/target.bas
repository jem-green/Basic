...
100 LET R=1: LET R1=57.296: LET P=3.14159
...
220 LET A=RND(1)*2*P: LET B=RND(1)*2*P: LET Q=INT(A*R1): LET W=INT(B*R1)
260 PRINT "RADIANS FROM X AXIS =";A;"   FROM Z AXIS =";B
280 LET P1=100000*RND(1)+RND(1): LET X=SIN(B)*COS(A)*P1: LET Y=SIN(B)*SIN(A)*P1
290 LET Z=COS(B)*P1
340 PRINT "TARGET SIGHTED: APPROXIMATE COORDINATES:  X=";X;"  Y=";Y;"  Z=";Z
345 LET R=R+1: IF R>5 THEN 390
350 ON R GOTO 355,360,365,370,375
355 LET P3=INT(P1*.05)*20: GOTO 390
360 LET P3=INT(P1*.1)*10: GOTO 390
365 LET P3=INT(P1*.5)*2: GOTO 390
370 LET P3=INT(P1): GOTO 390
375 LET P3=P1
...
420 LET A1=A1/R1: LET B1=B1/R1: PRINT "RADIANS FROM X AXIS =";A1;"  ";
425 PRINT "FROM Z AXIS =";B1
480 LET X1=P2*SIN(B1)*COS(A1): LET Y1=P2*SIN(B1)*SIN(A1): LET Z1=P2*COS(B1)
510 LET D=((X1-X)^2+(Y1-Y)^2+(Z1-Z)^2)^(1/2)
...
580 LET R=0: FOR I=1 TO 5: PRINT: NEXT I: PRINT "NEXT TARGET...": PRINT
590 GOTO 220
670 LET X2=X1-X: LET Y2=Y1-Y: LET Z2=Z1-Z: IF X2<0 THEN 730
