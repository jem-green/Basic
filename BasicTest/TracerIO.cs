...
namespace BasicTest
{
    public class TracerIO : IDefaultIO
    {
        ...
        public void Put(char c)
        {
            lock (_lockObject)
            {
                ...
                if (_cursor.Left > _consoleWidth)
                {
                    _cursor.Left = 1;
                    _cursor.Top++;
                    TracerLibrary.TraceInternal.Write("\n");
                    TracerLibrary.TraceInternal.Write(c.ToString());
                }
                else
                {
                    TracerLibrary.TraceInternal.Write(c.ToString());
            ...
            lock (_lockObject)
            {
                ...
                if (_cursor.Left > _consoleWidth)
                {
                    if (check.Length > 0)
                    {
                        ...
                        _cursor.Top++;
                        TracerLibrary.TraceInternal.Write("\n");
                        TracerLibrary.TraceInternal.Write(s);
                    }
                    else
                    {
                        _cursor.Left = 0;
        ...
        {
            TracerLibrary.TraceInternal.TraceError(e);
        }
